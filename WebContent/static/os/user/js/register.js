...
$(function() {
	...
	$("#btn_part1").click(function() {
		...
		$.ajax({
			type : "POST",
			url : baselocation + '/superlogin/sameregister',
			data : params,
			dataType : "json",
			success : function(result) {
				if (result.code == 1) {
                    $(".part4").children("h3").text("恭喜，您已注册成功！");
                    $(".part1").hide();
                    $(".part4").show();
                    ...
                    countdown({
                        ...
                        after : function() {
                            window.location.href = baselocation + '/superlogin/samelogin';
                        }
                    });
                    $(".message").hide();;
				} else {
...
$(function() {
	$("#btn_login").click(function() {
		...
		$.ajax({
			type : "POST",
			url : baselocation + '/superlogin/samelogin',
			data : {
				"username" : loginName,
			...
			success : function(result) {
				if (result.code == 1) {
					$(".message").hide();
					window.location.href = baselocation + '/server/toIndex';
				} else {
					$(".message").show();
					$(".message").children("label").text(result.message);
				}
...
$(function() {
	...
	$("#btn_Pswpart3").click(function() {
		...
		$.ajax({
			...
			success : function(result) {
				if (result.code == 1) {
					$(".part4").children("h3").text("恭喜" + result.data + "动力用户,您已成功找回密码,现在开始您的购物之旅吧!");
					$(".part3").hide();
					$(".part4").show();
					$(".step li").eq(2).addClass("on");
					countdown({
...
$(function() {
	$('#kaptchaImage').click(function() {
		$(this).attr('src', baselocation + '/superlogin/captcha-image.jpg?' + Math.floor(Math.random() * 100));
	})
});
